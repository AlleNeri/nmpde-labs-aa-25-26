...
Stokes::setup()
{
  ...
  // Initialize the finite element space.
  {
	...
	// Notice: unused in this example, but left here for completeness.
              ...
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
				  ...
				  cell_matrix(i, j) +=
					alpha *							// stabilization term alpha
					scalar_product(fe_values[velocity].value(i, q),
								   fe_values[velocity].value(j, q)) *
					fe_values.JxW(q); // product of the basis function values
									  // weighted for the quadrature.
