...
Stokes::assemble()
{
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
    {
	  ...
	  // They are zero on the only Neumann boundary, so we don't need to
	  // actually add them to the rhs.
  ...
  // Dirichlet boundary conditions.
  {
	...
	// I could have done it, in this case (in all the boundaries the second
	// component of the velocity is zero), but I perefered to stick with the
	// general case.

    // We interpolate first the zero velocity condition alone, then the non-zero 
    // conditions alone, so that the latter "win" over the former where the two
    // boundaries touch.
	
	// Gamma 0
	boundary_functions[0] = &velocity0;
