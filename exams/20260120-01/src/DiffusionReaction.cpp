...
void
DiffusionReaction::output(const unsigned int &iter) const
{
...
void
DiffusionReaction::apply_interface_dirichlet(const DiffusionReaction &other)
{
...
void
DiffusionReaction::apply_interface_neumann(DiffusionReaction &other)
{
  const auto interface_map = compute_interface_map(other);
...
DiffusionReaction::compute_interface_map(const DiffusionReaction &other) const
{
...
void
DiffusionReaction::apply_relaxation(const Vector<double> &old_solution,
                            const double         &lambda)
{
  solution *= lambda;
