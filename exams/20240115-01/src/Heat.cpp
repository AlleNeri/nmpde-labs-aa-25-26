...
Heat::assemble()
{
                  ...
                  cell_matrix(i, j) += theta *	// theta coefficient(time disc.)
					(
					  (
						...
						b_loc *				// b coefficient at quadrature point
						fe_values.shape_value(j, q) *
              ...
              cell_rhs(i) -= (1.0 - theta) *  // 1 - theta
							(
							 (
							   ...
							   b_loc *		  // b coefficient at quadrature
							   solution_old_values[q] *
							   fe_values.shape_grad(i, q) *
							   fe_values.JxW(q)
							 )
  ...
  // No Neumann boundary conditions: nothing to do here.
}
...
Heat::solve_linear_system()
{
  ...
  // Alternative preconditioner for unsymmetric systems.
